...
작성일: 2026-05-28  
목적: 회원가입 플로우 및 이후 A/B/C 앱뷰 화면 구현 전, 한국 모바일 앱 문맥에 맞는 레이아웃 구성 원칙을 정리한다.  
적용 범위: `/a|b|c/signup` 우선, 이후 home/tax/policies/chat 화면에도 공통 적용.


## 0. 구현 전 필수 확인 규칙

모든 페이지 구현자는 코드 수정 전에 이 문서의 핵심 결론과 구현 체크리스트를 먼저 확인한다. 이후 해당 페이지의 근거 문서(`설계문서.md`, `wireframes/`, `.omx/plans/abc-page-design-plan.md`)를 대조하고, 페이지별 구현 선택을 아래 원칙에 맞춘다.

- 한 화면 한 목적, 한 CTA 원칙을 깨지 않는다.
- 모바일 앱뷰 320~430px, 터치 타깃 48px, 하단 CTA/nav 겹침 방지를 먼저 본다.
- 요구사항/시나리오에 없는 기능성 요소를 추가하지 않는다.
- A/B/C는 콘텐츠가 아니라 시각 언어만 다르게 한다.
- 사용자 화면에 `A안`, `B안`, `C안`, `n안`, variant 설명명 같은 내부 실험 라벨을 절대 노출하지 않는다.
...
- 좌우 여백은 variant별 토큰을 쓰되, 모든 입력/카드의 시작점은 같은 x축에 맞춘다.
...
크몽의 앱 UI/UX 체크리스트는 화면 수, 구성 명세, 레퍼런스, UX 설계 여부, 브랜드/폰트 규정을 먼저 정리하면 작업 효율과 일관성이 높아진다고 설명한다. 이 repo에는 이미 `DESIGN.md`, `설계문서.md`, `wireframes/`, `.omx/plans/abc-page-design-plan.md`가 있으므로 구현은 이 네 가지를 기준으로 한다.
...
- 흰 배경, 선명한 blue CTA.
- 카드 수는 적게, 질문과 답변 영역을 넓게.
- 회원가입은 가장 실제 앱 같은 기본안으로 구현.

### B안: 정보 카드형 분석 앱

- light blue-gray 배경.
- 단계별 카드 stack, 상세 설명/완료 조건을 조금 더 구조적으로 노출.
- 가족/소득/주거 정보를 요약 카드처럼 읽게 만든다.

### C안: AI 비서형 앱

- soft panel, 말풍선형 안내.
- 질문이 대화처럼 이어지는 느낌.
- 추천 chip/친근한 안내 문구를 쓰되, 요구사항 밖 기능은 추가하지 않는다.
...
- [ ] 사용자 화면에 `A안/B안/C안/n안` 또는 variant 설명 라벨이 노출되지 않음.
- [ ] `npm run validate` 통과.
