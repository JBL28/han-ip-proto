 ...
 *
 * 이 파일은 메인 페이지의 큰 카드 3개(중요알림 / 오늘의 지식 한입 / 캘린더)를
 ...
 *   예: 추천 칩 간격은 .main2-benefit-row, 캘린더 내부 간격은 .main2-calendar-section.
 ...
 *       ui-calendar-caption, ui-list-title, ui-caption, ui-chip-title, ui-chip-caption.
...
export function BenefitAlertCard({ alert, recommendations, personaId, deductionDetailPath, visualSrc }) {
  return (
    <AppCard className="main2-alert-card">
      ...
      <h1 className="ui-alert-title">{alert.message}</h1>
      <p className="ui-body-muted">{alert.recommendationHeading}</p>
  ...
  return (
    <AppCard className="main2-knowledge-card">
      <div>
        ...
        <strong className="ui-knowledge-title">{knowledge.title}</strong>
...
export function PolicyCalendarCard({ calendar, personaId, month, selectedDate, onSelectDate, modifiers }) {
  return (
    <AppCard className="main2-calendar-section main2-calendar-card" aria-label="캘린더 정책 및 세금 일정">
      <div className="main2-section-head">
        <h2 className="ui-section-title">캘린더 (정책 + 세금)</h2>
        <button className="ui-action" type="button">전체 일정 보기 &gt;</button>
      </div>
      <div className="calendar-caption"><button type="button" aria-label="이전 달">&lt;</button><strong className="ui-calendar-caption">{calendar.caption}</strong><button type="button" aria-label="다음 달">&gt;</button></div>
      <DayPicker
        mode="single"
        month={month}
      ...
      <div className="main2-calendar-legend">
        <span className="ui-caption"><i className="legend-dot legend-dot--policy" />정책 신청기간</span>
        <span className="ui-caption"><i className="legend-dot legend-dot--tax" />세금 신고기간</span>
      </div>
    </AppCard>
  );
}
...
export function BenefitChip({ item, to }) {
  ...
  return (
    <Link className="main2-benefit-chip" to={to} aria-label={label} title={label}>
      ...
      <span className="ui-chip-caption">{item.description}</span>
    </Link>
  );
}
